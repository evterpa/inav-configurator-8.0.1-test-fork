...
function initGraphHelpers(selector, sampleNumber, heightDomain, $context) {
    var $el = $context ? $context.find(selector) : $(selector);
    var helpers = {selector: selector, targetElement: $el, dynamicHeightDomain: !heightDomain};
...
TABS.sensors.initialize = function (callback) {
    ...
    GUI.load(path.join(__dirname, "sensors.html"), function load_html() {
        ...
        if (!BitHelper.bit_check(FC.CONFIG.activeSensors, 4)) {
            checkboxes.eq(4).prop('disabled', true);
        }
        if (!BitHelper.bit_check(FC.CONFIG.activeSensors, 6)) {
        ...
        $('.tab-sensors .info .checkboxes input').on('change', function () {
            ...
            var wrappers = ['.gyro', '.accel', '.mag', '.altitude', '.sonar', '.airspeed', '.temperature', '.debug'];
            $('.wrapper' + wrappers[index]).toggle(enable);
        ...
        var gyro_data = initDataArray(3), accel_data = initDataArray(3),
            mag_data = initDataArray(3), altitude_data = initDataArray(2),
            sonar_data = initDataArray(1), airspeed_data = initDataArray(1);

        var temperature_data = [], debug_data = [];
        for (var i = 0; i < 8; i++) {
            temperature_data.push(initDataArray(1));
        ...
        var temperatureHelpers = [], debugHelpers = [];
        for (var i = 0; i < 8; i++) {
            temperatureHelpers.push(initGraphHelpers('#temperature' + (i + 1), samples_temperature_i));
            debugHelpers.push(initGraphHelpers('#debug' + (i + 1), samples_debug_i));
        }
...
TABS.sensors.initializeInContainer = function(containerSelector, callback) {
    ...
    $.get(path.join(__dirname, "sensors.html"), function(html) {
        $container.html(html);
        initializeSensorsInContainer($container, function() {
...
function initializeSensorsInContainer($container, callback) {
    i18n.localize();

    var checkboxes = $container.find('.info .checkboxes input');
    if (!BitHelper.bit_check(FC.CONFIG.activeSensors, 2)) {
        checkboxes.eq(2).prop('disabled', true);
    }
    if (!BitHelper.bit_check(FC.CONFIG.activeSensors, 4)) {
    ...
    $container.find('.info .checkboxes input').on('change', function () {
        var enable = $(this).prop('checked');
        var index = $(this).parent().index();
        var wrappers = ['.gyro', '.accel', '.mag', '.altitude', '.sonar', '.airspeed', '.temperature', '.debug'];
        $container.find('.wrapper' + wrappers[index]).toggle(enable);

        var checkboxes = [];
        $container.find('.info .checkboxes input').each(function () {
            checkboxes.push($(this).prop('checked'));
        });
        startPolling();
        store.set('graphs_enabled', checkboxes);
    ...
    if (graphs_enabled) {
        ...
        $container.find('.info input:lt(4):not(:disabled)').prop('checked', true).trigger('change');
    }
    ...
    var gyroHelpers = initGraphHelpers('#gyro', samples_gyro_i, [-2000, 2000], $container);
    var accelHelpers = initGraphHelpers('#accel', samples_accel_i, [-2, 2], $container);
    var magHelpers = initGraphHelpers('#mag', samples_mag_i, [-1, 1], $container);
    var altitudeHelpers = initGraphHelpers('#altitude', samples_altitude_i, $container);
    var sonarHelpers = initGraphHelpers('#sonar', samples_sonar_i, $container);
    var airspeedHelpers = initGraphHelpers('#airspeed', samples_airspeed_i, $container);

    var temperatureHelpers = [], debugHelpers = [];
    for (var i = 0; i < 8; i++) {
        temperatureHelpers.push(initGraphHelpers('#temperature' + (i + 1), samples_temperature_i, null, $container));
        debugHelpers.push(initGraphHelpers('#debug' + (i + 1), samples_debug_i, null, $container));
    }

    var raw_data_text_ements = {x: [], y: [], z: []};
    $container.find('.plot_control .x, .plot_control .y, .plot_control .z').each(function () {
        var el = $(this);
        if (el.hasClass('x')) raw_data_text_ements.x.push(el);
        else if (el.hasClass('y')) raw_data_text_ements.y.push(el);
        else raw_data_text_ements.z.push(el);
    ...
    var sensor_settings = store.get('sensor_settings', false);
    if (sensor_settings) {
        $container.find('select[name="gyro_refresh_rate"]').val(sensor_settings.rates.gyro);
        $container.find('select[name="gyro_scale"]').val(sensor_settings.scales.gyro);
        $container.find('select[name="accel_refresh_rate"]').val(sensor_settings.rates.accel);
        $container.find('select[name="accel_scale"]').val(sensor_settings.scales.accel);
        $container.find('select[name="mag_refresh_rate"]').val(sensor_settings.rates.mag);
        $container.find('select[name="mag_scale"]').val(sensor_settings.scales.mag);
        $container.find('select[name="baro_refresh_rate"]').val(sensor_settings.rates.baro);
        $container.find('select[name="sonar_refresh_rate"]').val(sensor_settings.rates.sonar);
        $container.find('select[name="airspeed_refresh_rate"]').val(sensor_settings.rates.airspeed);
        $container.find('select[name="debug_refresh_rate"]').val(sensor_settings.rates.debug);
    }

    function startPolling() {
        var rates = {
            'gyro': parseInt($container.find('select[name="gyro_refresh_rate"]').val(), 10),
            'accel': parseInt($container.find('select[name="accel_refresh_rate"]').val(), 10),
            'mag': parseInt($container.find('select[name="mag_refresh_rate"]').val(), 10),
            'baro': parseInt($container.find('select[name="baro_refresh_rate"]').val(), 10),
            'sonar': parseInt($container.find('select[name="sonar_refresh_rate"]').val(), 10),
            'airspeed': parseInt($container.find('select[name="airspeed_refresh_rate"]').val(), 10),
            'debug': parseInt($container.find('select[name="debug_refresh_rate"]').val(), 10)
        };
        var scales = {
            'gyro': parseFloat($container.find('select[name="gyro_scale"]').val()),
            'accel': parseFloat($container.find('select[name="accel_scale"]').val()),
            'mag': parseFloat($container.find('select[name="mag_scale"]').val())
        };

        var fastest = d3.min([rates.gyro, rates.accel, rates.mag]);
        store.set('sensor_settings', {'rates': rates, 'scales': scales});

        gyroHelpers = initGraphHelpers('#gyro', samples_gyro_i, [-scales.gyro, scales.gyro], $container);
        accelHelpers = initGraphHelpers('#accel', samples_accel_i, [-scales.accel, scales.accel], $container);
        magHelpers = initGraphHelpers('#mag', samples_mag_i, [-scales.mag, scales.mag], $container);
    ...
    $container.find('.rate select, .scale select').on('change', function () {
        startPolling();
    });

    startPolling();

    $container.find("a.debug-trace").on('click', function () {
        var debugWin = window.open("tabs/debug_trace.html", "receiver_msp", "width=500,height=510,menubar=no,contextIsolation=no,nodeIntegration=yes");
        debugWin.window.getDebugTrace = function () { return FC.DEBUG_TRACE || ''; };
    });
